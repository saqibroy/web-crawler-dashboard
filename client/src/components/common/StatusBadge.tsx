...
const StatusBadge = ({ status }: { status: AnalysisStatus }) => {
  ...
  const configs = {
    ...
    processing: { icon: Hourglass, bg: 'bg-yellow-100', text: 'text-yellow-800', label: 'Processing' },
    queued: { icon: Info, bg: 'bg-gray-100', text: 'text-gray-800', label: 'Queued' },
    cancelled: { icon: XCircle, bg: 'bg-orange-100', text: 'text-orange-800', label: 'Cancelled' },
  };
